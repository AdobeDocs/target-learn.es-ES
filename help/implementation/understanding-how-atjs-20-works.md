...
1. La llamada devuelve el Experience Cloud ID (ECID). Si el usuario está autenticado, otra llamada sincroniza el ID de cliente.
...
1. Los scripts de perfil se ejecutan y se alimentan en el [!UICONTROL Almacén de perfiles]. El Almacenamiento solicita audiencias de la [!UICONTROL Biblioteca de audiencias] que cumplan los requisitos (por ejemplo, audiencias compartidas de [!DNL Analytics], Audience Manager, etc.). Los [!UICONTROL atributos del cliente] se envían a [!UICONTROL almacén de perfiles] en un proceso por lotes.
...
1. [!DNL Analytics] datos enviados desde la página a [!UICONTROL Servidores de recopilación de datos]
...
1. La solicitud de notificación se ha enviado al [!DNL Target] [!UICONTROL Almacén de perfiles] para contar al visitante en la actividad e incrementar las métricas
1. Se han enviado [!DNL Analytics] datos de la SPA a los servidores de [!UICONTROL recopilación de datos]

1. Se han enviado los datos de [!DNL Target] desde el backend de [!DNL Target] a los servidores de [!UICONTROL recopilación de datos]. Se compararon los datos de [!DNL Target] con los datos de [!DNL Analytics] mediante el SDID y se procesaron en el almacén de informes de [!DNL Analytics]. Los datos de [!DNL Analytics] se pueden ver en [!DNL Analytics] y [!DNL Target] mediante los informes de A4T.
