...
Este artículo explica las modificaciones recomendadas para analizar las actividades [!DNL Auto-Target] en Workspace, que se basan en los siguientes conceptos clave:
...
* La atribución de ámbitos de retrospectiva de visita (también conocida como la &quot;ventana de retrospectiva de visita&quot; en el modelo de atribución prescrito) la utilizan los modelos ML de Adobe Target durante sus fases de formación, y el mismo modelo de atribución (no predeterminado) debería usarse idealmente al desglosar la métrica de objetivo.

## Creación del panel A4T para [!DNL Auto-Target] en Workspace

Para crear un informe de A4T para [!DNL Auto-Target], comience con el panel típico de Analytics para Target en Workspace, como se muestra a continuación, o comience con una tabla improvisada. A continuación, realice las siguientes selecciones:

1. **[!UICONTROL Experiencia]** de control: Puede elegir cualquier experiencia; sin embargo, posteriormente anularemos esta opción. Tenga en cuenta que para las actividades [!DNL Auto-Target], la experiencia de control es realmente una estrategia de control, que puede ser a) Servir aleatoriamente entre todas las experiencias o b) Servir una sola experiencia (esta opción se realiza en el momento de crear la actividad en Adobe Target). Incluso si optó por la opción (b) (su actividad [!DNL Auto-Target] designó una experiencia específica como Control), debe seguir el enfoque descrito en este artículo para analizar A4T para actividades [!DNL Auto-Target].
1. **[!UICONTROL Métrica de normalización]**: Seleccione Visitas.
1. **[!UICONTROL Métricas de éxito]**: Aunque puede seleccionar cualquier métrica en la que desee informar, generalmente debe ver los informes sobre la misma métrica que se eligió para la optimización durante la creación de la actividad en Adobe Target.
...
*pngFigura 2: El informe de referencia recomendado para las  [!DNL Auto-Target] actividades. Este informe se ha configurado para comparar el tráfico segmentado (servido por el modelo ML de ensamblado) con el tráfico de control*

>[!NOTE]
>
>Actualmente, los números de Alza y Confianza no están disponibles para dimensiones de Control y Segmentación para informes de A4T para Segmentación automática. Hasta que no se agregue la compatibilidad, el alza y la confianza se pueden calcular manualmente descargando la [calculadora de confianza](https://experienceleague.adobe.com/docs/target/assets/complete_confidence_calculator.xlsx?lang=en).

## Información sobre los desgloses de métricas a nivel de experiencia
...
Al analizar una actividad [!DNL Auto-Target], elija siempre Visitas como métrica de normalización predeterminada. [!DNL Auto-Target] la personalización selecciona una experiencia para un visitante una vez por visita (formalmente, una vez por sesión de Adobe Target), lo que significa que la Experiencia mostrada a un usuario puede cambiar en cada visita. Por lo tanto, si utilizamos visitantes únicos como métrica de normalización, un solo usuario puede terminar viendo varias experiencias (en diferentes visitas), lo que podría provocar tasas de conversión confusas.
...
### Para crear el segmento correcto:

1. Seleccione la opción **[!UICONTROL Componentes > Crear segmento]** en la barra de herramientas de Workspace.
1. Introduzca un **[!UICONTROL Título]** para el segmento. En el ejemplo que se muestra a continuación, el segmento se llama [!DNL "Hit with specific Auto-Target activity"].
1. Arrastre la dimensión **[!UICONTROL Actividades de Target]** a la sección **[!UICONTROL Definición]** del segmento.
1. Utilice el operador **[!UICONTROL equals]**.
1. Busque su actividad de Target específica.
1. Seleccione el icono de engranaje y seleccione **[!UICONTROL Attribution model > Instance]** como se muestra en la figura siguiente.
...
Una vez creado el segmento, podemos utilizarlo para filtrar la métrica Visitas, de modo que solo incluya las visitas en las que el usuario interactuó con la actividad de Target.
...
1. Libere el segmento. El filtro se aplicará a esa métrica.
...
## Avanzado: Atribución de ámbito de visita para métricas de objetivo

La integración de A4T permite que el modelo ML de [!DNL Auto-Target] esté *entrenado* utilizando los mismos datos de evento de conversión que se utilizan para *generar informes de rendimiento* en Adobe Analytics. Sin embargo, hay ciertas hipótesis que deben emplearse para interpretar estos datos al entrenar los modelos ML, que difieren de los supuestos predeterminados realizados durante la fase de notificación en Adobe Analytics.
...
## Paso final: Tasas de conversión con atribución con ámbito de visita y normalización de visitas filtrada
...
1. Arrastre el segmento creado anteriormente (denominado &quot;Visita con actividad [!DNL Auto-Target] específica&quot; en este artículo) para esta actividad [!DNL Auto-Target] específica.
